...
function EventoTooltip({ ev }: { ev: Evento }) {
  const cor = getCor(ev);
  return (
    <div style={{ position:"absolute", bottom:"calc(100% + 6px)", left:0, zIndex:50, minWidth:200, maxWidth:260, background:"white", border:"1px solid #E4E6EA", borderRadius:10, padding:10, boxShadow:"0 8px 24px rgba(0,0,0,.12)", fontFamily:"'Urbanist',sans-serif", pointerEvents:"none" }}>
...
function EventoBtn({ ev, onClick }: { ev: Evento; onClick: () => void }) {
  ...
  return (
    <div style={{ position:"relative" }} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}>
      <button onClick={onClick} style={{ width:"100%", textAlign:"left", padding:"3px 6px", borderRadius:6, border:`1px solid ${cor}33`, background:`${cor}14`, fontSize:11, cursor:"pointer", display:"flex", alignItems:"center", gap:5, fontFamily:"'Urbanist',sans-serif", overflow:"hidden" }}>
    ...
    <div onClick={(e) => { if(e.target===e.currentTarget) onClose(); }}
      ...
      <div style={{ background:"white", borderRadius:16, width:"100%", maxWidth, maxHeight:"90vh", overflow:"auto", boxShadow:"0 24px 60px rgba(0,0,0,.25)" }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"16px 20px", borderBottom:"1px solid #F0F5FF" }}>
          <h3 style={{ margin:0, fontSize:16, fontWeight:700, color:"#0F172A", fontFamily:"'Urbanist',sans-serif" }}>{title}</h3>
...
const lbl: React.CSSProperties = { fontSize:11, fontWeight:700, textTransform:"uppercase" as const, color:"#64748B", letterSpacing:".06em", display:"block", marginBottom:5, fontFamily:"'Urbanist',sans-serif" };
    ...
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <button onClick={() => setNovoOpen(true)}
            style={{ height:36, padding:"0 18px", borderRadius:100, background:"#3B82F6", border:"none", color:"white", fontSize:13, fontWeight:700, cursor:"pointer", display:"flex", alignItems:"center", gap:6, fontFamily:"'Urbanist',sans-serif" }}>
      ...
      <div style={{ display:"flex", flexWrap:"wrap", gap:12, alignItems:"center", justifyContent:"space-between" }}>
        <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
          <select value={anoFiltro} onChange={(e) => irParaAno(Number(e.target.value))}
            style={{ height:34, borderRadius:100, border:"1px solid #E4E6EA", padding:"0 12px", fontSize:13, fontFamily:"'Urbanist',sans-serif", outline:"none", background:"white", cursor:"pointer" }}>
          ...
          <div style={{ minWidth:180, textAlign:"center", fontWeight:700, fontSize:14, fontFamily:"'Urbanist',sans-serif", color:"#0F172A" }}>{titulo}</div>
          ...
          <button onClick={() => setCursor(new Date())}
            style={{ height:34, padding:"0 14px", borderRadius:100, border:"1px solid #E4E6EA", background:"white", fontSize:13, fontWeight:600, cursor:"pointer", fontFamily:"'Urbanist',sans-serif" }}>Hoje</button>
        </div>

        <div style={{ display:"inline-flex", padding:3, background:"#F1F5F9", borderRadius:100 }}>
          {(["mes","semana"] as const).map(v => (
            <button key={v} onClick={() => setView(v)}
              style={{ height:28, padding:"0 14px", borderRadius:100, border:"none", fontSize:12, fontWeight:600, cursor:"pointer", background:view===v?"white":"transparent", color:view===v?"#0F172A":"#64748B", boxShadow:view===v?"0 1px 4px rgba(0,0,0,.08)":"none", fontFamily:"'Urbanist',sans-serif" }}>
              {v==="mes"?"Mês":"Semana"}
            </button>
          ))}
        </div>
      ...
      <div style={{ borderRadius:12, border:"1px solid #E4E6EA", background:"white", overflow:"hidden" }}>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", background:"#F8FAFC" }}>
          {DIAS_SEM.map(d => (
            <div key={d} style={{ padding:"8px 10px", fontSize:11, fontWeight:700, color:"#64748B", textAlign:"center", textTransform:"uppercase", letterSpacing:".06em", fontFamily:"'Urbanist',sans-serif" }}>{d}</div>
          ))}
        </div>
            ...
            return (
              <div key={i} style={{ borderTop:"1px solid #F0F5FF", borderLeft:i%7===0?"none":"1px solid #F0F5FF", padding:6, minHeight:view==="semana"?260:100, display:"flex", flexDirection:"column", gap:4, background:sameMonth?"white":"#F8FAFC" }}>
                <div style={{ display:"flex", justifyContent:"flex-end" }}>
                  <span style={{ height:24, width:24, display:"inline-flex", alignItems:"center", justifyContent:"center", borderRadius:6, fontSize:12, fontWeight:600, background:isHoje?"#3B82F6":"transparent", color:isHoje?"white":sameMonth?"#0F172A":"#94A3B8", fontFamily:"'Urbanist',sans-serif" }}>{d.getDate()}</span>
                </div>
                <div style={{ display:"flex", flexDirection:"column", gap:3, overflow:"hidden" }}>
                  ...
                  {evs.length > MAX_VIS && (
                    <span style={{ fontSize:10, color:"#64748B", padding:"0 6px", fontFamily:"'Urbanist',sans-serif" }}>+{evs.length-MAX_VIS} mais</span>
                  )}
                </div>
              </div>
            );
      ...
      <div style={{ display:"flex", flexWrap:"wrap", gap:12, padding:14, borderRadius:12, border:"1px solid #E4E6EA", background:"white" }}>
        {(Object.keys(TIPO_LABEL) as EventoTipo[]).filter(t => t!=="outros").map(t => (
          <span key={t} style={{ display:"inline-flex", alignItems:"center", gap:6, fontSize:12, color:"#374151", fontFamily:"'Urbanist',sans-serif" }}>
          ...
          return (
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              ...
              <div style={{ display:"flex", flexWrap:"wrap", gap:8, alignItems:"center" }}>
                <span style={{ display:"inline-flex", alignItems:"center", gap:6, padding:"4px 10px", borderRadius:100, background:`${cor}14`, color:cor, fontSize:11, fontWeight:700, textTransform:"uppercase", letterSpacing:".06em", fontFamily:"'Urbanist',sans-serif" }}>
                  <span style={{ width:8, height:8, borderRadius:2, background:cor }} />
                  {getLabel(evSel)}
                </span>
                <span style={{ padding:"4px 10px", borderRadius:100, background:"#F1F5F9", color:"#475569", fontSize:11, fontWeight:600, fontFamily:"'Urbanist',sans-serif" }}>
                  {evSel.visibilidade==="interno"?"Interno":evSel.visibilidade==="empresa"?`Empresa: ${evSel.empresa}`:"Para todos"}
                </span>
              </div>

              <div style={{ display:"flex", flexDirection:"column", gap:6, fontSize:13, color:"#374151", fontFamily:"'Urbanist',sans-serif" }}>
              ...
              {(evSel.convidados??[]).length > 0 && (
                <div>
                  <div style={{ display:"flex", alignItems:"center", gap:6, fontSize:11, fontWeight:700, textTransform:"uppercase", letterSpacing:".06em", color:"#64748B", marginBottom:8, fontFamily:"'Urbanist',sans-serif" }}>
                    <Users size={12} /> Convidados
                  </div>
                  <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                    {(evSel.convidados??[]).map((c,i) => (
                      <div key={i} style={{ display:"flex", alignItems:"center", gap:8, padding:8, borderRadius:8, border:"1px solid #F0F5FF", fontFamily:"'Urbanist',sans-serif" }}>
                        <span style={{ width:26, height:26, borderRadius:6, background:"linear-gradient(135deg,#031D38,#3B82F6)", display:"inline-flex", alignItems:"center", justifyContent:"center", color:"white", fontSize:10, fontWeight:700 }}>{c.iniciais}</span>
                        ...
                        {c.resposta.status==="pendente" && (
                          <button onClick={() => { setConvidadoModal({ev:evSel,convidado:c}); setJustificativa(""); }}
                            style={{ fontSize:11, height:24, padding:"0 8px", borderRadius:6, border:"1px solid #E4E6EA", background:"white", cursor:"pointer", fontFamily:"'Urbanist',sans-serif" }}>Responder</button>
                        )}
              ...
              <div style={{ display:"flex", gap:8, flexWrap:"wrap", paddingTop:10, borderTop:"1px solid #F0F5FF" }}>
                <a href={googleCalendarUrl(evSel)} target="_blank" rel="noopener noreferrer"
                  style={{ display:"flex", alignItems:"center", gap:5, height:32, padding:"0 12px", borderRadius:8, border:"1px solid #E4E6EA", background:"white", fontSize:12, fontWeight:600, color:"#374151", textDecoration:"none", fontFamily:"'Urbanist',sans-serif" }}>
                  <ExternalLink size={14} /> Google Calendar
                </a>
                <button onClick={() => downloadICS(evSel)}
                  style={{ display:"flex", alignItems:"center", gap:5, height:32, padding:"0 12px", borderRadius:8, border:"1px solid #E4E6EA", background:"white", fontSize:12, fontWeight:600, color:"#374151", cursor:"pointer", fontFamily:"'Urbanist',sans-serif" }}>
                  <Download size={14} /> Apple / .ics
                </button>
                <button onClick={() => setEvSel(null)}
                  style={{ marginLeft:"auto", height:32, padding:"0 14px", borderRadius:8, border:"1px solid #E4E6EA", background:"white", fontSize:13, fontWeight:600, cursor:"pointer", color:"#374151", fontFamily:"'Urbanist',sans-serif" }}>Fechar</button>
              </div>
            </div>
          );
        })()}
      ...
      <Modal open={!!convidadoModal} onClose={() => setConvidadoModal(null)} title="Responder convite" maxWidth={400}>
        {convidadoModal && (
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            <p style={{ margin:0, fontSize:13, color:"#475569", fontFamily:"'Urbanist',sans-serif" }}>
            ...
            <div style={{ display:"flex", gap:8, justifyContent:"flex-end" }}>
              <button onClick={() => setConvidadoModal(null)} style={{ height:34, padding:"0 14px", borderRadius:8, border:"1px solid #E4E6EA", background:"white", fontSize:13, cursor:"pointer", color:"#374151", fontFamily:"'Urbanist',sans-serif" }}>Cancelar</button>
              <button onClick={() => responderConvite(false)} disabled={!justificativa.trim()}
                style={{ height:34, padding:"0 14px", borderRadius:8, border:"none", background:"#EF4444", color:"white", fontSize:13, fontWeight:700, cursor:"pointer", fontFamily:"'Urbanist',sans-serif", opacity:!justificativa.trim()?0.5:1 }}>Recusar</button>
              <button onClick={() => responderConvite(true)}
                style={{ height:34, padding:"0 14px", borderRadius:8, border:"none", background:"#10B981", color:"white", fontSize:13, fontWeight:700, cursor:"pointer", fontFamily:"'Urbanist',sans-serif" }}>Aceitar</button>
            </div>
          </div>
        )}
      </Modal>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          ...
          <div>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:5 }}>
              {USUARIOS.map(u => (
                <label key={u.nome} style={{ display:"flex", alignItems:"center", gap:8, cursor:"pointer", fontSize:13, color:"#374151", fontFamily:"'Urbanist',sans-serif" }}>
                  ...
                  <span style={{ width:24, height:24, borderRadius:6, background:"linear-gradient(135deg,#031D38,#3B82F6)", display:"inline-flex", alignItems:"center", justifyContent:"center", color:"white", fontSize:9, fontWeight:700 }}>{u.iniciais}</span>
                  {u.nome}
                </label>
              ))}
            </div>
          ...
          <div style={{ display:"flex", justifyContent:"flex-end", gap:8, paddingTop:8, borderTop:"1px solid #F0F5FF" }}>
            <button onClick={() => setNovoOpen(false)} style={{ height:34, padding:"0 14px", borderRadius:8, border:"1px solid #E4E6EA", background:"white", fontSize:13, cursor:"pointer", color:"#374151", fontFamily:"'Urbanist',sans-serif" }}>Cancelar</button>
            <button onClick={criarEvento} disabled={!fTitulo.trim()||!fData}
              style={{ height:34, padding:"0 18px", borderRadius:8, border:"none", background:"#3B82F6", color:"white", fontSize:13, fontWeight:700, cursor:"pointer", fontFamily:"'Urbanist',sans-serif", opacity:(!fTitulo.trim()||!fData)?0.5:1 }}>Criar evento</button>
          </div>
        </div>
      </Modal>
    </div>
